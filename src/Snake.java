...
class SnakePanel extends JPanel implements Runnable, KeyListener {
	...
	private void checkCollision() {
		...
		if( headRect.x <= 5 || headRect.x >= 590 || headRect.y <= 5 || headRect.y >= 365) {
			isCrashed = true;
		}
	...
	private void checkSelfCollision() {
		Rectangle r1 = new Rectangle(headRect.x, headRect.y, headRect.bredde, headRect.hoyde);
		Snakeelement next = headRect.getNext();
		
		//Traverse the snake and compare each element with the head
		while(next != null) {
			Rectangle r2 = new Rectangle(next.x, next.y, next.bredde, next.hoyde);
	...
	public void run() {
		...
		/* The algorithm for MOVING the snake is as follows:
		 * 1. Move the head
		 * 2. Insert new element in heads old place 
		 * 3. New elements tail is heads old tail
		 * 4. Heads new tail is the new element
		 * 5. Remove the last element of the snake
		 * */
		while(!isCrashed) {
	...
	public void keyPressed(KeyEvent e) {
		...
		case KeyEvent.VK_UP:
			...
			if(direction != MOVING_DOWN) { direction = MOVING_UP; }
			break;
			
		case KeyEvent.VK_DOWN:
			
			if(direction != MOVING_UP) { direction = MOVING_DOWN; }
			break;
			
		case KeyEvent.VK_LEFT:
			
			if(direction != MOVING_RIGHT) { direction = MOVING_LEFT; }
			break;
			
		case KeyEvent.VK_RIGHT:	
			
			if(direction != MOVING_LEFT) { direction = MOVING_RIGHT; }
			break;
